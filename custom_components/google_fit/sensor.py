...
class GoogleFitHeartRateSensor(GoogleFitSensor):
    ...
    def update(self):
        """Extracts the relevant data points for from the Fitness API."""
        heartrate_datasources = self._get_datasources(
...
class GoogleFitSleepSensor(GoogleFitSensor):
    ...
    def update(self):
        ...
        sleep_dataset = self._client.users().sessions().list(
            userId='me', fields='session', startTime=starttime, endTime=endtime).execute()
        starts = []
        ends = []
        deep_sleep = []
        light_sleep = []
        ...
        for point in sleep_dataset["session"]:
            if int(point["activityType"]) == 72:
                ...
                if point["name"].startswith('Deep'):
                    deep_sleep_start = datetime.fromtimestamp(
                        int(point["startTimeMillis"]) / 1000)
                    deep_sleep_end = datetime.fromtimestamp(
                        int(point["endTimeMillis"]) / 1000)
                    _LOGGER.debug("Deep Sleep dataset Total %s",
                                  (deep_sleep_end - deep_sleep_start))
                    deep_sleep.append(deep_sleep_end - deep_sleep_start)
                elif point["name"].startswith('Light'):
                    light_sleep_start = datetime.fromtimestamp(
                        int(point["startTimeMillis"]) / 1000)
                    light_sleep_end = datetime.fromtimestamp(
                        int(point["endTimeMillis"]) / 1000)
                    _LOGGER.debug("Light Sleep dataset Total %s",
                                  (light_sleep_end - light_sleep_start))
